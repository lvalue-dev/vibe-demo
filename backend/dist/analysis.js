...
function analyze(closes, volumes, currentPrice, priceChangeRate) {
    ...
    if (volumeRatio && volumeRatio >= 1.5) {
        score += 20;
        reasons.push(`거래량이 평균 대비 ${volumeRatio.toFixed(1)}배 증가 - 관심 집중`);
    }
    if (priceChangeRate > 0) {
        score += 20;
        reasons.push(`전일 대비 +${(priceChangeRate * 100).toFixed(2)}% 상승 중`);
    }
    else
        reasons.push(`전일 대비 ${(priceChangeRate * 100).toFixed(2)}% 하락 중`);
    if (volatility < 0.05) {
        score += 20;
    ...
    const risk = (score < 40 || volatility >= 0.05) ? 'HIGH' : score < 60 ? 'MEDIUM' : 'LOW';
    const riskLabel = risk === 'HIGH' ? '높음' : risk === 'MEDIUM' ? '보통' : '낮음';
    return { score, recommendation, recommendationLabel, risk, riskLabel, reasons, ma5, ma20, volumeRatio, avgVolume5, avgVolume20 };
}
